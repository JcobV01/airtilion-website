...
const ButtonPrice = () => {
    ...
    return (
        <div className='flex items-center justify-center gap-[25px] text-[17px] tracking-[1.5px] font-light mt-[60px]'>
            <span>Netto</span>
            <div>
                <input type="checkbox" id="toggle" className='sr-only' checked={checked} onChange={handleToogle} />
                <label htmlFor="toggle" className='block w-16 h-8 rounded-[30px] cursor-pointer relative' style={{
                    ...
                    <span className={`absolute top-1 left-1 w-6 h-6 rounded-full transition-transform duration-300 ease-in-out ${checked ? 'translate-x-8' : ''}`} style={{
