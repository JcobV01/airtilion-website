...
const Cards = ({ offer, isNetto, handleOpenDialog }) => {
    ...
    return (
        <div className={`flex items-center h-screen w-full p-4 ${flexClass} xl:flex-col xl:gap-[50px] xl:h-full sm:p-0 xxl:h-[100%] xxl:mt-[60px]`}>
                        ...
                        <div className="relative rounded-[10px] backdrop-blur-[5px] bg-[rgba(27,27,27,0.40)] flex flex-col h-full px-[20px] fold:pb-[50px]">
                            ...
                            {offer.length >= 3 && index === 1 && (
                                <div className="absolute top-5 right-[-18px] py-[5px] px-[30px] rounded-[11.5px] sm:relative sm:text-center sm:mx-[0px] sm:right-0" style={{ background: 'radial-gradient(50% 50% at 50% 50%, #222 0%, #1E1E1E 100%)' }}>
                                    <span className='text-[#FFDE98] text-[12px] tracking-[1px] font-light xxl:text-[16px]'>Najlepszy wybór</span>
                                </div>
                            )}
                            ...
                            <h6 className={`font-medium text-[#E2B350] mt-[25px] mb-[20px] ${activeIndex === index ? 'text-[25px] tracking-[2.5px] xxl:text-[35px]' : 'text-[20px] tracking-[2px] xxl:text-[30px]'} xl:text-[25px] xl:tracking-[2.5px] sm:text-[20px] sm:my-[15px] fold:text-center`}>{price}<span className={`font-light text-[#E2B350] ${activeIndex === index ? 'text-[16px] tracking-[1.6px] xxl:text-[26px]' : 'text-[13px] tracking-[1.3px] xxl:text-[23px]'} xl:text-[16px] xl:tracking-[1.6px] sm:text-[13px]`}>&nbsp;{isNetto ? 'netto' : 'brutto'}</span></h6>
                            ...
                            <div className='flex items-center justify-center mt-[25px]'>
                                ...
                                <span className={`font-light transition ${activeIndex === index ? 'text-[18px] tracking-[1.8px] mx-[20px] xxl:text-[22px]' : 'text-[15px] tracking-[1.5px] mx-[10px] xxl:text-[19px]'} xl:text-[18px] xl:tracking-[1.8px] xl:mx-[20px] sm:text-[15px] sm:tracking-[1.5px] sm:mx-[10px] fold:text-[12px] fold:tracking-[1px]`}>Zawartość pakietu</span>
                                <span className='w-[60px] h-[1px] bg-[rgba(226,179,80,0.50);] fold:w-[20px] xxl:w-[90px]'></span>
                            </div>
                            ...
                            <div className='flex justify-center'>
                                <Link href={cards.link} className={`absolute bottom-2 transition text-[#ECD8AD] font-light ${activeIndex === index ? 'text-[14px] tracking 1.3px xxl:text-[18px]' : 'text-[12px] tracking 1px xxl:text-[16px]'} xl:text-[12px] xl:tracking-[1px] sm:text-[10px] sm:text-center`}>Poznaj pełne szczegóły pakietu&nbsp;<span className={`transition text-[#E2B350] font-medium ${activeIndex === index ? 'text-[14px] tracking 1.3px' : 'text-[12px] tracking 1px'} xl:text-[12px] xl:tracking 1px fold:text-[10px]`}>"{cards.linkText}"</span></Link>
                            </div>
