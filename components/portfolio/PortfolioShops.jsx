      ...
      {displayedItems.map((item, index) => (
        <div key={index} className="w-[250px] h-[500px] relative flex flex-col items-center justify-center portfolio-item-hovered duration-500 p-[20px] cursor-pointer" onClick={() => openProjectHandle(item._id)}>
          <Image
            ...
            alt={`Tło ${item.Name}`} />
          <Image src={`https://api.airtilion.com/uploads/projects/${item.Client}/${item.Logo.file}`} width={0} height={0} sizes="100vw" alt={`logo ${item.Name}`} className="w-auto h-[50px] z-10 absolute top-[50px] duration-500 xxl:h-[60px]" />
