        ...
        <div className='space-y-[32px]'>
            {
                data?.elements.map((element, index) => (
                    <Accordion open={open === element.id} icon={<Icon id={element.id} open={open} />} key={index}>
                        <AccordionHeader onClick={() => handleOpen(element.id)} className="text-[16px] tracking-[2.5px] font-semibold border-solid border-b-2 border-[#D9D9D9] pb-[12px] sm:text-[14px]">{element.question}</AccordionHeader>
